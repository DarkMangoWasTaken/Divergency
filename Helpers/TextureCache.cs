...
namespace DivergencyMod.Helpers
{
    public sealed class TextureCache : ILoadable
    {
        ...
        private void InnerLoadArrayProperty(PropertyInfo p)
        {
            ...
            {
                arr[i] = ModContent.Request<Texture2D>("DivergencyMod/" + folderSpace + "/Trail");
            }
        ...
        {
            string folderSpace = InnerGetPath(p);
            p.SetMethod.Invoke(null, new object[] { ModContent.Request<Texture2D>("DivergencyMod/" + folderSpace + "/Trail") });
        }
