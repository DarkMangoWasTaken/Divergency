...
namespace DivergencyMod.Dusts.Particles
{
    public class FlameParticle : Particle
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color lightColor)
        {
            ...
            Texture2D tex2 = Request<Texture2D>("DivergencyMod/Dusts/Particles/FlameParticle").Value;
            Texture2D tex3 = Request<Texture2D>("DivergencyMod/Dusts/Particles/FlameParticle").Value;

            float alpha = timeLeft <= 20 ? 1f - 1f / 20f * (20 - timeLeft) : 1f;
            if (alpha < 0f) alpha = 0f;
            Color color = Color.Multiply(new(0.46f, 0.26f, 0.71f, 0), alpha);
            ...
            spriteBatch.Draw(tex3, position - Main.screenPosition, new Rectangle(0, 0, tex3.Width, tex3.Height), color, ai[3].InRadians().AngleLerp((ai[3] + 90f).InRadians(), (120f - timeLeft) / 30f), new Vector2(tex3.Width / 2f, tex3.Height / 2f), 0.01f * Scale, SpriteEffects.None, 0f);
